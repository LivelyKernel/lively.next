...
defineMode("python", function(conf, parserConf) {
  ...
  function tokenBase(stream, state) {
    ...
    if (stream.sol() && _top(state).type == "py") {
      var scopeOffset = _top(state).offset;
      if (stream.eatSpace()) {
  ...
  function pushPyScope(state) {
    ...
    state.scopes.push({offset: _top(state).offset + conf.indentUnit,
                       type: "py",
                       align: null})
  }
  ...
  function tokenLexer(stream, state) {
    ...
    if (current == ":" && !state.lambda && _top(state).type == "py")
      pushPyScope(state);
  ...
  var external = {
    ...
    indent: function(state, textAfter) {
      if (state.tokenize != tokenBase)
        return state.tokenize.isString ? passIndent : 0;

      var scope = _top(state), closing = scope.type == textAfter.charAt(0)
      if (scope.align != null)
