...
// await updateHTMLStuff();
async function updateHTMLStuff() {

  var {html: html1, resource: r1} = await $world.get("login widget")._htmlExport();
  var {html: html2, resource: r2} = await $world.get("register widget")._htmlExport();

  html1 = html1.replace('style="', 'style="display: none; ');
  html2 = html2.replace('style="', 'style="display: none; ');
...
function setupForHTMLPage() {
  ...
  async function tryRegister() {
    let userNameInput = registerWidget.querySelector(".user-name-input"),
        passwordInput = registerWidget.querySelector(".password-input"),
        passwordInput2 = registerWidget.querySelector(".password-input-2"),
        emailInput = registerWidget.querySelector(".email-input"),
  ...
  async function tryLogin() {
    let userNameInput = loginWidget.querySelector(".user-name-input"),
        passwordInput = loginWidget.querySelector(".password-input"),
        username = userNameInput.value,
        password = passwordInput.value;
    ...
    if (error) { window.confirm(error); return; }
  ...
  async function continueAsGuest() {
    let reg = UserRegistry.current,
        stored = reg.loadUserFromLocalStorage(authServerURL),
        user = stored.isGuestUser ? stored : await reg.login(ClientUser.guest);
    resolveWithUser(user);
  }

  function setupRegisterWidget() {
    let registerButton = registerWidget.querySelector(".register-button"),
        cancelButton = registerWidget.querySelector(".cancel-button");
    cancelButton.onclick = () => switchToLoginWidget();
    registerButton.onclick = () => tryRegister();
    registerWidget.onkeydown = (evt) => {
      if (evt.keyCode === 13/*Enter*/) tryRegister();
      else if (evt.keyCode === 27/*Escp*/) switchToLoginWidget();
    }
  }

  function setupLoginWidget() {
    let loginButton = loginWidget.querySelector(".login-button"),
        registerButton = loginWidget.querySelector(".register-button"),
        guestButton = loginWidget.querySelector(".guest-button");
    ...
    loginWidget.onkeydown = (evt) => {
      if (evt.keyCode === 13/*Enter*/) tryLogin();
      else if (evt.keyCode === 27/*Escp*/) {}
    }
  }
