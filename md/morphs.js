...
this.markdownToHTMLPositionMap.map((ea, i) => `${i}: ${Math.round(ea)}`).join("\n")
  ...
  smoothScrollStep() {
    ...
    for (let i = this._smoothScroll.length; i--; ) {
      let spec = this._smoothScroll[i],
          [m, scrollToY, scrollDeltaY, startTime] = spec,
          node = m.env.renderer.getNodeForMorph(m),
          deltaY = Math.abs(node.scrollTop - scrollToY);
  ...
  computeMarkdownToHTMLPositionMap() {
    ...
    let lastMdLine = this.markdownSource.split("\n").length,
        parent = lineEls[0].parentElement,
        ...
        lineElsByMdLines = Array.from(lineEls).reduce((heightMap, el) => {
    ...
    for (let nextMdLine, mdLine = 0; mdLine < lineElsByMdLines.length-1; mdLine++) {
      for (let j = mdLine+1; j < lineElsByMdLines.length; j++)
        if (undefined !== lineElsByMdLines[j]) { nextMdLine = j; break;}
      let steps = nextMdLine - mdLine,
          heightDiff = lineElsByMdLines[nextMdLine] - lineElsByMdLines[mdLine],
          step = heightDiff / steps;
      for (let j = mdLine+1; j < nextMdLine; j++)
        lineElsByMdLines[j] = (lineElsByMdLines[j-1] || 0) + step;
      mdLine = nextMdLine-1;
    }
  ...
  syncMarkdownScrollToHTMLScroll(editor = this.markdownEditor) {
    ...
    function sigmoid(x) {
      let L = nRowsVisible, k = .7, x0 = nLines/2;
  ...
  syncHTMLScrollToMarkdownScroll(editor = this.markdownEditor) {
    ...
    var t = this.domNode.parentNode.scrollTop,
        mdLineClosest = markdownToHTMLPositionMap.reduce((min, height, i) => {
