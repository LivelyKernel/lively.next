...
import StorageResource, { StorageResourceBackend } from "../storage-resource.js";
...
describe("lively.storage resource", () => {
  ...
  beforeEach(async () => {
    await StorageResourceBackend.ensure(testProjectSpec, {host});
  });

  afterEach(() => {
    StorageResourceBackend.removeHost(host)
  });

  it("can read a file", async () => {
    await StorageResourceBackend.ensure({"test-location": {"foo": "foo bar"}}, {host});
    StorageResourceBackend.named(host)
    var r = resource(`local-storage://${host}/test-location/foo`);
    await r.read();
  ...
  // 
  // it("mkdir", async () => {
  //   var r = resource(`local-storage://${host}/bar/`);
  //   await r.mkdir();
  ...
  //   var r = resource(`local-storage://${host}/`);
  ...
  //     var r = resource(`local-storage://${host}/sub-dir`);
  // 
  //     expect((await r.dirList('infinity')).map(ea => ea.url)).deep.equals([
  //       r.join("file2.js").url,
  ...
  //     var r = resource(`local-storage://${host}/test-location/`),
