...
const dir = System.decanonicalize("lively.modules/tests/"),
      testProjectDir = dir + "files-for-eval-test/",
      testProjectSpec = {
...
describe("lively.modules aware eval", () => {
  ...
  it("of var being exported", async () => {
    ...
    var result = await runEval("var y = 2;", {asString: true, System: S, targetModule: module2.id});
    expect(result.value).to.not.match(/error/i);
    expect(m2.y).to.equal(2, "file2.js not updated");
    var result2 = await runEval("y;", {System: S, targetModule: module1.id});
    expect(result2.value).to.equal(2);
  ...
  it("of new export", async () => {
    var m1 = await S.import(module1.id),
        m2 = await S.import(module2.id),
        result = await runEval("export var xxx = 99;", {asString: true, System: S, targetModule: module2.id});
    ...
    var result2 = await runEval("import { xxx } from './file2.js'; xxx", {System: S, targetModule: module1.id});
  ...
  it("reload module dependencies", async () => {
    ...
    var source = await module3.source();
    source = source.replace(/(z = )([0-9]+)/, "$12");
    var result = await runEval(source, {asString: true, System: S, targetModule: module3.id});
    expect(result.value).to.not.match(/error/i);
    module3.unloadDeps();
    var m = await S.import(module1.id);
  ...
  describe("async", () => {
    ...
    it("awaits async function", async () => {
      ...
      var result = await runEval("await foo(3)", {System: S, targetModule: module4.id});
      await expect(result).property("value").to.equal(3);
    })

    it("nests await", async () => {
      await S.import(module4.id)
      var result = await runEval("await ('a').toUpperCase()", {System: S, targetModule: module4.id});
