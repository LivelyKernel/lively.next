...
function ignoreFormatCompare (result, expected) {
  return expect(stringify(parse(result))).equals(stringify(parse(expected)));
}
...
function classTemplate (className, superClassName, methodString, classMethodString, classHolder, moduleMeta, useClassHolder = true, start, end, evalId) {
  ...
  var ${useClassHolder ? '__lively_class__' : 'Foo'} = ${classFunctionHeader}(__first_arg__) {
...
bar;`);
  ...
  describe('es6', () => {
    ...
    describe('manual import', () => {
      let opts = { es6ExportFuncId: '_moduleExport', es6ImportFuncId: '_moduleImport', captureObj: nodes.id('_rec') };
