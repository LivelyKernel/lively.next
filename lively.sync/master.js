...
export class Master {
  ...
  sendMeta (connection, msg) {
    if (!connection || !connection.metaChannel) { throw new Error(`${this}: No (meta) connection when sending meta message ${i(msg)}`); }
  ...
  async handleLockAndUnlockRequest (msg, metaChannel) {
    let err; let lock = msg.request === 'lock';
  ...
  receiveOpsFromClient (ops, client, connection) {
    ...
    let opsForTransform = this.findOpsForTransform(ops[0]);
    let transformed = ops.map(op => this.transform(op, opsForTransform).transformedOp);
  ...
  broadcast (ops, sender) {
    ...
    let sourceCon = this.connectionFor(sender);
    let priorityCons = arr.without(this.connections, sourceCon);
