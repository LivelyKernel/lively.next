...
describe('diff and patch', () => {
  it('returns hunk for row', function () {
    ...
    let patch = FilePatch.read(patchString);
  ...
  it('returns hunk for row of multiple patched', function () {
    ...
    let patches = FilePatch.readAll(patchString);
  ...
  it('parse patch', function () {
    ...
    let expectedHunkString = '@@ -2,3 +2,3 @@\n' +
      " of the world's largest, open, production economies. Even with a market\n" +
      '-capitalization of $2.5 billion, the bitcoin economy is dwarfed by $15 trillion\n' +
      '+capitalization of $2.5 billion, the bitcoin economy is dwarfed hey by $15 trillion\n' +
      ' economies such as the U.S. Therefore, sudden and large increases in the user';
  ...
  it('create hunk from selected rows', function () {
    let origHunk = '@@ -2,4 +2,5 @@ xxx yyy\n' +
           ...
           ' foo bar baz\n' +
           ' har har har';
    let hunk = new FilePatchHunk().read(origHunk.split('\n'));
    let result, expected;
  ...
  it('patch string from rows for reverse', function () {
    let patch, result, expected, patchString;
  ...
  it('get changed lines', function () {
    ...
    let patch = FilePatch.read(diff);
    let lines = patch.changesByLines();
    let expected = [
  ...
  it('successful patch', function () {
    ...
    let patch = FilePatch.read(diff);
    let a = arr.range(0, 400).join('\n') + '\n';
    let b = a.replace('\n97\n', '\n')
      .replace('\n100\n', '\n')
      .replace('\n200\n', '\nXXX\nYYY\nZZZ\n')
      .replace('\n300\n', '\n300X\n');
    let result = patch.patch(a);
  ...
  it('print diff reverse', function () {
    let orig = arr.range(0, 10).join('\n') + '\n';
    let changed = orig.replace('\n5\n', '\nHello\nWorld\n');
    let diff = 'Index: foo\n' +
           '--- foo\n' +
           '+++ foo\n' +
           '@@ -2,9 +2,10 @@\n' +
    ...
    let resultForward = patch.createPatchString();
    expect(diff).equals(resultForward, 'normal diff');

    let resultReverse = patch.createPatchString(true);
  ...
  it('successful patch reverse', function () {
    ...
    let patch = FilePatch.read(diff).reverse();
    let result = patch.patch(changed);
...
`;
  expect('reads patch', () => {
    let patch = FilePatch.read(diffA);
