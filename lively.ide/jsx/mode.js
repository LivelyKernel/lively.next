...
import { passIndent, defineMode, defineMIME, registerHelper, startState, copyState, getMode } from '../editor-modes.js';
...
function copyContext (context) {
  return new Context(copyState(context.mode, context.state),
    context.mode,
    context.depth,
...
defineMode('jsx', function (config, parserConfig) {
  ...
  function xmlToken (stream, state, cx) {
    ...
    if (stream.peek() == '{') {
      ...
      let indent = flatXMLIndent(cx.state); let xmlContext = cx.state.context;
      ...
      state.context = new Context(startState(jsMode, indent),
    ...
    const style = xmlMode.token(stream, cx.state); const cur = stream.current(); let stop;
