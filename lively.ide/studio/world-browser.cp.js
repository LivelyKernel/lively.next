...
import { add, without } from 'lively.morphic/components/core.js';
...
class ProjectPreviewModel extends WorldPreviewModel {
  ...
  initWithProject (project) {
    const {
      preview, timestamp, title, description, owner
    } = this.ui;
    this.opacity = 0.5;
    ...
    authorName = authorName.startsWith('guest') ? 'guest' : authorName;
    owner.value = [project.projectRepoOwner, { fontSize: 13, fontWeight: 'bold', paddingTop: '1px' }];
...
const ProjectPreviewTile = component(WorldPreviewTile, {
  ...
  submorphs: [{
    ...
    }, 'timestamp'), without('version button')]
