    ...
    return {
      ...
      color: {
        set (c) {
          ...
          const [h, s, b] = c ? c.toHSB() : [];
          this.setProperty('color', c);
          if (!c) return;
          if (colorView && hashView) {
            colorView.fill = c;
            hashView.textString = c.toHexString();
            hashView.fontColor = Color.hsb(h, s + 0.5 > 1 ? s - 0.5 : s + 0.5, b + 0.5 > 1 ? b - 0.5 : b + 0.5);
          }
        }
      },
      submorphs: {
        after: ['color'],
        initialize () {
          const [h, s, b] = (this.color || Color.transparent).toHSB();
          this.submorphs = [
            new Morph({ fill: this.color, extent: pt(80, 50) }),
            new Text({
              textString: `${h.toFixed()}, ${s.toFixed(2)}, ${b.toFixed(2)}`,
...
class HarmonyDisplay extends Morph {
  update (colorPalette) {
    ...
    const colors = colorPalette.harmony.chord({ hue, saturation, brightness });
    if (colors.length != this.submorphs.length) {
      this.submorphs = colors.map(c => {
        const f = new ColorHarmonyField({ color: c });
...
class PivotColorControl extends Ellipse {
  ...
  onDrag (evt) {
    ...
    const angle = num.toRadians(h);
    const currentPos = Point.polar(50 * s, angle);
    const newPos = currentPos.addPt(evt.state.dragDelta);
    var h = num.toDegrees(newPos.theta());
    var h = h < 0 ? h + 360 : h;
    var s = Math.min(newPos.r() / 50, 1);
...
class HarmonyVisualizer extends Image {
  ...
  update (colorPalette) {
    ...
    const pivotControl = this.get('pivot control');
    const [hue, saturation, brightness] = colorPalette.pivotColor.toHSB();
    const angle = num.toRadians(hue);
    const chord = colorPalette.harmony.chord({ hue, saturation, brightness });
    const colorPoints = chord.map(c => {
      const [h, s, _] = c.toHSB();
      const angle = num.toRadians(h);
...
export class ColorPalette extends Morph {
  ...
  selectSolidMode () {
    const duration = 200;
    const solidPalette = this.get('solidColorPalette');
    const paletteView = this.get('paletteView');
  ...
  getPalette (colors, mod) {
    ...
    const width = cols * this.colorFieldWidth;
    const height = mod * this.colorFieldWidth;
    const paddedColors = [...colors, ...arr.withN((cols * mod) - colors.length, null)];
    return {
      extent: pt(width, height),
      name: 'paletteContainer',
      submorphs: paddedColors.map(c => {
        const field = new ColorPaletteField({
  ...
  switchPalette ({ name, colors, mod }) {
    const colorPalette = this.get('solidColorPalette'); const [_, config] = colorPalette.submorphs;
  ...
  paletteConfig () {
    ...
    const config = {
      name: 'paletteConfig',
      styleClasses: ['paletteFormatter'],
      submorphs: [
        selector = new DropDownSelector({
          isHaloItem: true,
          name: 'paletteSelector',
          selectedValue: 'flatDesign',
          padding: 4,
