...
defineMode("pug", function (config) {
  ...
  function State() {
    ...
    this.jsState = cmStartState(jsMode);
  ...
  State.prototype.copy = function () {
    ...
    res.jsState = cmCopyState(jsMode, this.jsState);

    res.innerMode = this.innerMode;
    if (this.innerMode && this.innerState) {
      res.innerState = cmCopyState(this.innerMode, this.innerState);
    }
    ...
    if (state.isAttrs) {
      ...
      if (state.inAttributeName && stream.match(/^[^=,\)!]+/)) {
        if (stream.peek() === '=' || stream.peek() === '!') {
          ...
          state.jsState = cmStartState(jsMode);
  ...
  function innerMode(stream, state, force) {
    if (stream.indentation() > state.indentOf || (state.innerModeForLine && !stream.sol()) || force) {
      if (state.innerMode) {
        if (!state.innerState) {
          state.innerState = state.innerMode.startState ? cmStartState(state.innerMode, stream.indentation()) : {};
        }
