...
import { passIndent, defineMode, defineMIME, mimeModes } from '../editor-modes.js';
...
defineMode('xml', function createMode (editorConf, config_) {
  const { indentUnit } = { ...defaultConfig, ...editorConf };
  const config = {};
  ...
  function closeTagNameState (type, stream, state) {
    if (type == 'word') {
      ...
      if (state.context && state.context.tagName != tagName &&
          config.implicitlyClosed.hasOwnProperty(state.context.tagName)) { popContext(state); }
...
if (!mimeModes.hasOwnProperty('text/html')) { defineMIME('text/html', { name: 'xml', htmlMode: true }); }
