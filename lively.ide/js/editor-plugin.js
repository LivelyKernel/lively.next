...
export default class JavaScriptEditorPlugin extends CodeMirrorEnabledEditorPlugin {
  ...
  async getMenuItems (items) {
    ...
    const ref = nav.resolveIdentifierAt(editor, editor.cursorPosition);
    if (ref) {
      jsItems.push(
  ...
  backend (envMixin) {
    const env = this.sanatizedJsEnv(envMixin);
  ...
  parse (astType = null) {
    ...
    const options = { withComments: true, allowReturnOutsideFunction: true };
  ...
  undeclaredVariables (astType = null) {
    const { textMorph: morph, parser } = this;
    const doc = morph.document;
    const knownGlobals = this.evalEnvironment.knownGlobals || [];
    const parsed = this.parse(astType);
