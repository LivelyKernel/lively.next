...
class MarkdownCompiler {
  compileToHTML (src, options = {}) {
    ...
    let md = markdownIt(options).use(externalizeLinksPlugin).use(addSourceLineMappingPlugin).use(markdownCheckbox).use(markdownAttrs).use(markdownCaption, { dataType: true, figcaption: true }).use(html5Media).use(markdownMermaid);
  ...
  parse (editor, options) {
    options.html = true; // allow usage of HTML tags in all markdown
    let md = markdownIt(options).use(externalizeLinksPlugin).use(markdownCheckbox).use(markdownCaption, { dataType: true, figcaption: true }).use(markdownAttrs).use(html5Media).use(markdownMermaid);
