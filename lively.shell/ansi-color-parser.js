...
export function convertToTextStyles (string) {
  ...
  */
  ...
  // -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
  let indexOffset = 0; let result = [0, []]; let currentAttributes = [];
  string = string.replace(ansiAttributesRegexp, (match, index) => {
    // match can look like this "\033[31m" or this "\033[1;31m"
    let attributeCodes = match.slice(2, match.length - 1).split(';');
    let styles = Object.keys(ansiAttributes).map((code) =>
  ...
  let lastIndex = result[result.length - 2];
