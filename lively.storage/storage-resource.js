...
export default class LivelyStorageResource extends Resource {
  ...
  async mkdir () {
    ...
    if (spec) {
      if (!spec.isDirectory) { throw new Error(`${this.url} already exists and is a file (cannot mkdir it!)`); }
      return this;
    }
    let t = Date.now();
  ...
  async remove () {
    ...
    let db = this.db;
    let matching = await db.docList({ startkey: thisPath, endkey: thisPath + '\uffff' });
  ...
  async dirList (depth = 1, opts = {}) {
    ...
    let { exclude } = opts;
    let prefix = this.pathWithoutQuery();
    let children = [];
    let docs = await this.db.getAll({ startkey: prefix, endkey: prefix + '\uffff' });

    if (depth === 'infinity') depth = Infinity;

    for (let i = 0; i < docs.length; i++) {
      let doc = docs[i];
      let { _id: path, isDirectory } = doc;
      if (!path.startsWith(prefix) || prefix === path) continue;
      let trailing = path.slice(prefix.length);
      let childDepth = trailing.includes('/') ? trailing.match(slashRe).length + 1 : 1;
      if (childDepth > depth) {
