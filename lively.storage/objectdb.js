...
const sha1 = (function sha1_setup () { function r (r) { if (void 0 === r) return o(!1); let e = o(!0); return e.update(r), e.digest(); } function e () { let r = f.createHash('sha1'); return { update: function (e) { return r.update(e); }, digest: function () { return r.digest('hex'); } }; } function t (r) { function e (r) { if (typeof r === 'string') return t(r); let e = r.length; h += 8 * e; for (let n = 0; n < e; n++)o(r[n]); } function t (r) { let e = r.length; h += 8 * e; for (let t = 0; t < e; t++)o(r.charCodeAt(t)); } function o (r) { a[y] |= (255 & r) << g, g ? g -= 8 : (y++, g = 24), y === 16 && u(); } function f () { o(128), (y > 14 || y === 14 && g < 24) && u(), y = 14, g = 24, o(0), o(0), o(h > 0xffffffffff ? h / 1099511627776 : 0), o(h > 4294967295 ? h / 4294967296 : 0); for (let r = 24; r >= 0; r -= 8)o(h >> r); return i(s) + i(c) + i(v) + i(p) + i(d); } function u () { for (var r = 16; r < 80; r++) { let e = a[r - 3] ^ a[r - 8] ^ a[r - 14] ^ a[r - 16]; a[r] = e << 1 | e >>> 31; } let t; let n; let o = s; let f = c; let u = v; let i = p; let g = d; for (r = 0; r < 80; r++) { r < 20 ? (t = i ^ f & (u ^ i), n = 1518500249) : r < 40 ? (t = f ^ u ^ i, n = 1859775393) : r < 60 ? (t = f & u | i & (f | u), n = 2400959708) : (t = f ^ u ^ i, n = 3395469782); let h = (o << 5 | o >>> 27) + t + g + n + (0 | a[r]); g = i, i = u, u = f << 30 | f >>> 2, f = o, o = h; } for (s = s + o | 0, c = c + f | 0, v = v + u | 0, p = p + i | 0, d = d + g | 0, y = 0, r = 0; r < 16; r++)a[r] = 0; } function i (r) { for (var e = '', t = 28; t >= 0; t -= 4)e += (r >> t & 15).toString(16); return e; } let a; var s = 1732584193; var c = 4023233417; var v = 2562383102; var p = 271733878; var d = 3285377520; var y = 0; var g = 24; var h = 0; return a = r ? n : new Uint32Array(80), { update: e, digest: f }; } let n, o, f; return typeof process === 'object' && typeof process.versions === 'object' && process.versions.node && process.__atom_type !== 'renderer' ? (f = typeof System !== 'undefined' ? System._nodeRequire('crypto') : require('crypto'), o = e) : (n = new Uint32Array(80), o = t), r; })();
...
export default class ObjectDB {
  ...
  static named (name, options = {}) {
    ...
    if (typeof options.snapshotLocation === 'string') {
      try { options.snapshotLocation = resource(options.snapshotLocation); } catch (err) {
        options.snapshotLocation = resource(System.baseURL)
  ...
  snapshotResourceFor (commit) {
    ...
    let rest = commit.content.slice(2);
  ...
  async snapshotObject (type, name, object, snapshotOptions, commitSpec, preview, ref, expectedPrevVersion) {
    ...
    let serializeFn = x => x;
    let snapshot = await serializeFn(object, snapshotOptions);
  ...
  async objects (optType) {
    ...
    let result = {};
    for (let type in stats) { result[type] = Object.keys(stats[type]); }
    return result;
  }
  ...
  async objectStats (objectType, objectName) {
    ...
    let queryOpts = { reduce: true, group: true };
    ...
    try {
      ...
      for (let { key: objectTypeAndName, value: { count, max: newest, min: oldest } } of rows) {
        ...
        let statsOfType = statsByType[type] || (statsByType[type] = {});
  ...
  async getCommits (type, objectName, ref = 'HEAD', limit = Infinity) {
    ...
    let commitDB = this.__commitDB || await this._commitDB();
    let commits = await commitDB.getDocuments(history.map(ea => ({ id: ea })));
    return commits;
  }
  ...
  async commit (type, name, snapshot, commitSpec, preview, ref = 'HEAD', expectedPrevVersion) {
    ...
    let versionData = await this.versionGraph(type, name);
    let ancestor = versionData ? versionData.refs[ref] : null;
    let ancestors = ancestor ? [ancestor] : [];
    ...
    let snapshotJson = snapshotIsHash ? null : snapshot ? JSON.stringify(snapshot) : null;
  ...
  async loadSnapshot (type, name, commitOrId, ref = 'HEAD') {
    ...
    if (!commit) { throw new Error(`Cannot find commit to loadSnapshot for ${type}/${name} (using ${commitOrId})`); }
    return this.snapshotResourceFor(commit).readJson();
  }

  async revert (type, name, ref, toCommitId) {
    let versionDB = this.__versionDB || await this._versionDB();
    let history = await versionDB.get(type + '/' + name);
  ...
  _createCommitFromSpec (commit, isHashed = false) {
    ...
    let commitHash = sha1(JSON.stringify(hashObj));
  ...
  async versionGraph (type, objectName) {
    let versionDB = this.__versionDB || await this._versionDB();
    let graph = await versionDB.get(type + '/' + objectName);
  ...
  async _log (type, objectName, ref = 'HEAD', limit = Infinity) {
    ...
    let version = data.refs.HEAD; let history = [];
    while (true) {
      if (history.includes(version)) { throw new Error('cyclic version graph???'); }
  ...
  async _findTimestampedVersionsOfObjectNamed (objectName, options = {}) {
    ...
    let startkey = `${objectName}\u0000${descending ? endTime : startTime}`;
    let endkey = `${objectName}\u0000${descending ? startTime : endTime}`;
    let objectDB = this.__commitDB || await this._commitDB();
    let { rows } = await objectDB.pouchdb.query('nameAndTimestamp_index', {
      ...options,
      descending,
      include_docs,
      startkey,
  ...
  async exportToDir (exportDir, nameAndTypes, copyResources = false, includeDeleted = false) {
    ...
    if (!nameAndTypes) {
      ...
      for (let { refs, history, _id } of versions) {
        ...
        let commitIds = Object.keys(history);
        let commits = await this.getCommitsWithIds(commitIds);
    ...
    for (let { refs, history, currentExportDir, commits, name, type } of backupData) {
      if (!includeDeleted) { commits = commits.filter(ea => !ea.deleted); }

      let resourcesForCopy = copyResources
        ? commits.map(commit => {
          if (commit.deleted || commit._deleted || !commit.content) return null;
          delete commit._rev;
          let from = this.snapshotResourceFor(commit);
          let to = currentExportDir.join(from.parent().name() + '/' + from.name());
          return { from, to };
        }).filter(Boolean)
        : [];
      ...
      for (let { from, to } of resourcesForCopy) { await from.copyTo(to); }
    }
  }
  ...
  async exportToSpecs (nameAndTypes, includeDeleted = false) {
    ...
    if (!nameAndTypes) { // = everything
      ...
      for (let type in stats) {
        for (let name in stats[type]) { nameAndTypes.push({ type, name }); }
      }
    }
  ...
  async importFromDir (importDir, overwrite = false, copyResources = false) {
    ...
    let { snapshotLocation } = this; let importSpecs = [];
    ...
    async function findImportDataIn (dir) {
      ...
      let snapshotDirs = copyResources
  ...
  async importFromSpecs (specs, overwrite = false, copyResources = false) {
    if (!overwrite) {
      ...
      for (let { type, name } of specs) {
        if (await versionDB.get(`${type}/${name}`)) { throw new Error(`Import failed: object ${type}/${name} already exists and overwrite is not allowed`); }
      }
    }

    for (let spec of specs) { await this.importFromSpec(spec, true, copyResources); }
  ...
  async importFromSpec (spec, overwrite = false, copyResources = false) {
    let versionDB = this.__versionDB || await this._versionDB();
    let commitDB = this.__commitDB || await this._commitDB();
    let { snapshotLocation } = this;
    let { type, name, commits, history, snapshotDirs } = spec;

    if (!overwrite && await versionDB.get(`${type}/${name}`)) { throw new Error(`Import failed: object ${type}/${name} already exists and overwrite is not allowed`); }
  ...
  async importFromResource (type, name, resource, commitSpec, purgeHistory = false) {
    ...
    if (purgeHistory && await this.has(type, name)) { await this.delete(type, name, false); }
  ...
  async resolveConflict (arg) {
    ...
    let { resolved, delete: del, kind, id } = arg; let db;
    if (kind === 'versions') {
  ...
  async getDiff (remoteCommitDBOrName, remoteVersionDB) {
    ...
    let localVersionDB = this.__versionDB || await this._versionDB();
    let commitDiff = await localCommitDB.diffWith(remoteCommitDB);
    let versionDiff = await localVersionDB.diffWith(remoteVersionDB);
    let local = await Promise.all(versionDiff.inLeft.map(async ea => ({ id: ea.id, doc: await localVersionDB.get(ea.id) })));
    let allRemoteVersions = arr.groupBy(await remoteVersionDB.getAll(), d => d._id);
    let remote = await Promise.all(versionDiff.inRight.map(async ea => ({ id: ea.id, doc: allRemoteVersions[ea.id] })));
    let changed = await Promise.all(versionDiff.changed.map(async ea => ({ ...ea.left, docA: await localVersionDB.get(ea.left.id), docB: await remoteVersionDB.get(ea.right.id) })));
    let allRemoteCommits = arr.groupBy(await remoteCommitDB.getAll(), d => d._id);
    let localCommits = []; let remoteCommits = []; let changedCommits = [];
    ...
    let remoteCommitTypeAndNames = remoteCommits.map(ea => obj.select(ea, ['_id', 'name', 'type']));
    let changedCommitTypeAndNames = changedCommits.map(ea => obj.select(ea, ['_id', 'name', 'type']));
  ...
  async delete (type, name, dryRun = true) {
    ...
    let { rows } = await objectDB.query('nameAndTimestamp_index', opts);
    ...
    // 3. history to delete
    let versionDB = this.__versionDB || await this._versionDB();
    let { _id, _rev } = await versionDB.get(type + '/' + name);
    let deletedHist = { _id, _rev, _deleted: true };
  ...
  async deleteCommit (commitOrId, dryRun = true, ref = 'HEAD') {
    ...
    let versionDB = this.__versionDB || await this._versionDB();
    let objectDB = this.__commitDB || await this._commitDB();
    let { name, type, _id } = commit;
    let resources = (commit.deleted || commit._deleted || !commit.content)
      ? []
      : [this.snapshotResourceFor(commit)];
    let commitDeletions = [{ ...commit, _deleted: true }];
    let hist = await versionDB.get(type + '/' + name);
...
class Synchronization {
  ...
  async _startReplicationAndCopy () {
    ...
    let versionDB = fromObjectDB.__versionDB || await fromObjectDB._versionDB();
    let commitDB = fromObjectDB.__commitDB || await fromObjectDB._commitDB();
    let {
      _commitdb_indexes, _versiondb_indexes,
      snapshotLocation: fromSnapshotLocation
    } = fromObjectDB;
    let versionChangeListener; let commitChangeListener;

    this.method = method;

    let commitNameTypeFilter = _commitdb_indexes.find(ea => ea.name === 'nameTypeFilter');
    let versionNameTypeFilter = _versiondb_indexes.find(ea => ea.name === 'nameTypeFilter');
    ...
    let opts = {
      live, retry
      // conflicts: true,
    }; let commitOpts = { ...opts }; let versionOpts = { ...opts };
    ...
    let versionReplication = versionDB[method](remoteVersionDB, versionOpts);
    let snapshotReplication = {
      copyCalls: 0,
      copyCallsWaiting: 0,
      nFilesToCopy: 0,
      nFilesCopied: 0,
          ...
          return toResource.exists().then(toExists => {
            ...
            return fromResource.exists().then(fromExists => {
              ...
              return tryCopy(0).then(result => {
                ...
                if (!snapshotReplication.stopped && snapshotReplication.nFilesCopied % 10 === 0) { console.log(`${this} copied ${snapshotReplication.nFilesCopied} of ${snapshotReplication.nFilesToCopy} snapshots`); }
                return result;
              });
            });
...
function checkArg (name, value, spec) {
  ...
  if (typeof spec === 'string') {
    ...
    let actualClass = value ? value.constructor.name : '';
    let types = spec.split('|');
    let matches = types.some(t => actualType === t || actualClass === t);
...
function checkArgs (args, specs, testFn) {
  for (let key in specs) { checkArg(key, args[key], specs[key]); }
...
export var ObjectDBInterface = {
  ...
  async describe (method) {
    ...
    try {
      if (!this._methodSpecs) {
        ...
        let parsed = parse(src, { withComments: true });
        let entities = categorizer.findDecls(parsed);
        this._methodSpecs = entities.filter(ea => ea.parent && ea.parent.name === 'ObjectDBInterface');
      }

      return method
        ? methodNameAndParametersAndDescription(this._methodSpecs, method)
        : this._methodSpecs
    ...
    function methodNameAndParametersAndDescription (methodSpecs, name) {
      ...
      let body = methodSpec.node.value.body;
      let stmts = body.body || [];
      let comment = (body.comments || []).find(ea => ea.end < stmts[0].start);
      let doc = { name, parameters: [], sideEffect: false, returns: null, description: '' };

      if (comment && comment.text.trim()) {
        let text = string.changeIndent(comment.text, ' ', 0);
        let commentLines = text.split('\n');
      ...
      for (let stmt of stmts) {
        if (Path('declarations.0.init.callee.name').get(stmt) !== 'checkArgs') { continue; }
  ...
  async fetchCommits (args) {
    ...
    let versionDB = db.__versionDB || await db._versionDB();

    let versionQueryOpts = {};
    let refsByTypeAndName = {};
    if (typesAndNames) {
    ...
    let versions = await versionDB.getAll(versionQueryOpts); let commitIds = [];
    ...
    if (!includeDeleted) { commits = commits.filter(ea => ea && !ea.deleted); }
    if (filterFn) {
      try {
        let fn = eval(`(${filterFn})`);
        if (typeof fn !== 'function') { throw new Error(`${filterFn} does not eval to a function!`); }
        let filteredCommits = commits.filter(fn);
        if (!Array.isArray(filteredCommits)) { throw new Error(`${filterFn} does not return an array!`); } else commits = filteredCommits;
      } catch (err) {
  ...
  async exists (args) {
    ...
    let db = await ObjectDB.find(dbName);
    let hist = await db.versionGraph(type, name);
  ...
  async fetchLog (args) {
    ...
    if (commit) {
      ...
      if (!type || !name) {
        ...
        if (!realCommit) { throw new Error(`fetchLog: specified commit ${commit} but no commit with this id is in the database!`); }
    ...
    let currentCommit = startCommitId; let result = [];
  ...
  async exportToDir (args) {
    ...
    let { db: dbName, url, nameAndTypes, copyResources, includeDeleted } = checkArgs(args, {
      ...
      includeDeleted: 'boolean|undefined'
    }); let db = await ObjectDB.find(dbName); let exportDir;
    if (!db) throw new Error(`db ${dbName} does not exist`);
    try { exportDir = resource(url); } catch (err) { exportDir = resource(System.baseURL).join(url); }
  ...
  async importFromDir (args) {
    ...
    let { db: dbName, url, overwrite, copyResources } = checkArgs(args, {
      ...
      copyResources: 'boolean|undefined'
    }); let db = await ObjectDB.find(dbName); let importDir;
    if (!db) throw new Error(`db ${dbName} does not exist`);
    try { importDir = resource(url); } catch (err) { importDir = resource(System.baseURL).join(url); }
  ...
  async importFromResource (args) {
    ...
    }); let db = await ObjectDB.find(dbName); let res;
    if (!db) throw new Error(`db ${dbName} does not exist`);
  ...
  async synchronize (args) {
    ...
    let db2 = await ObjectDB.named(otherDB, { snapshotLocation: otherDBSnapshotLocation });
    let remoteCommitDB = await db2._commitDB();
    let remoteVersionDB = await db2._versionDB();
    let toSnapshotLocation = db2.snapshotLocation;
    let opts = {
      replicationFilter: onlyTypesAndNames ? { onlyTypesAndNames } : undefined,
      retry: true,
      live: true
    };
    let rep = db1[method](remoteCommitDB, remoteVersionDB, toSnapshotLocation, opts);
