...
describe('database access', () => {
  ...
  describe('get all', () => {
    ...
    it('retrieves specified docs by and rev', async () => {
      let doc1 = await db.set('foo', { x: 23 });
      let doc2 = await db.set('foo', { x: 24 });
      let docs = await db.getDocuments([{ id: 'foo', rev: doc1._rev }, { id: 'foo', rev: doc2._rev }]);
      expect(docs).length(2);
  ...
  describe('revisions', () => {
    it('reads revision list', async () => {
      ...
      let doc2_1 = await db.set('some-doc-2', { a: 'heelo' });
      let doc2_2 = await db.set('some-doc-2', { a: 'world' });
      let doc1_2 = await db.set('some-doc-1', { x: 24 });
...
describe('database migration', () => {
  ...
  it('converts documents', async () => {
    ...
    let status = await migrationDB.migrate(doc => ({ newField: doc.oldField }));
    let migrated = await migrationDB.getAll();
  ...
  it('can keep docs unchanged', async () => {
    let original = await migrationDB.getAll();
    let status = await migrationDB.migrate((doc, i) =>
...
describe('backup', () => {
  ...
  afterEach(async () => {
    ...
    if (backupDB) { expect(await backupDB.destroy()).deep.equals({ ok: true }); }
  });
