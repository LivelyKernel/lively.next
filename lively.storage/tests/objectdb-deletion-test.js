...
describe('deletions in ObjectDB', function () {
  ...
  describe('deletion of all versions', () => {
    ...
    it('snapshots removed', async () => {
      ...
      let commits2Before = await objectDB.getCommits('world', world2.name);
      let expectedBefore = commits1Before.concat(commits2Before).reduce((expected, ea) => {
        return {
          ...expected,
          [objectDB.snapshotLocation.join(ea.content.slice(0, 2) + '/' + ea.content.slice(2) + '.json').url]: true
        };
      }, {});
      let actualBefore = (await snapshotLocation.dirList('infinity', { exclude: ea => ea.isDirectory() }))
      ...
      let commits2After = await objectDB.getCommits('world', world2.name);
      let expectedAfter = commits1After.concat(commits2After).reduce((expected, ea) => {
        return {
          ...expected,
          [objectDB.snapshotLocation.join(ea.content.slice(0, 2) + '/' + ea.content.slice(2) + '.json').url]: true
        };
      }, {});
      let actualAfter = (await snapshotLocation.dirList('infinity', { exclude: ea => ea.isDirectory() }))
        .reduce((actual, ea) => ({ ...actual, [ea.url]: true }), {});
