...
export class SVGMorph extends Morph {
  static get properties () {
    return {
      extent: { defaultValue: pt(800, 450) },
  ...
  selectElement (target) {
    ...
    if (this.target && this.target.id === target.id && this.target.selected) return;
  ...
  updateSVGafterControlPointDrag (controlPoint, moveDelta) {
    ...
    } else if (this.isControlPoint(controlPoint)) {
  ...
  isControlPoint (controlPoint) {
    return controlPoint.id.startsWith('control-point');
  }
  ...
  initializeSVGPath (svgPath) {
    ...
    const viewBox = this.svgPath.viewBox.baseVal;
    let ratio = parseFloat(viewBox.width) / parseFloat(viewBox.height);
    // not every svg has viewbox width and height defined
    ratio = ratio || parseFloat(svgPath.getAttribute('width')) / parseFloat(svgPath.getAttribute('height'));
