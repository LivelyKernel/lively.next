...
function computeHashIdOfEvent (evt) {
  const { key, ctrlKey, altKey, shiftKey, metaKey } = evt;
...
function decodeKeyIdentifier (identifier, keyCode) {
  ...
  const id = identifier;
  const unicodeDecodeRe = /u\+?([\d\w]{4})/gi;
  const unicodeReplacer = function (match, grp) { return String.fromCharCode(parseInt(grp, 16)); };
...
var Keys = {
  ...
  keyComboToEventSpec (keyCombo, flags) {
    ...
    const keyMods = dedasherize(keyCombo);
    const modsToEvent = {
      shift: 'shiftKey',
      control: 'ctrlKey',
      ctrl: 'ctrlKey',
    ...
    for (let i = keyMods.length - 1; i >= 0; i--) {
      const mod = keyMods[i];
      const modEventFlag = modsToEvent[mod.toLowerCase()];
    ...
    const trailing = arr.last(keyMods);

    // 3. determine the key code and key string of the event.
    const fnKey = canonicalizeFunctionKey(trailing);
    if (fnKey) {
  ...
  eventToKeyCombo (evt, options) {
    ...
    const { ignoreModifiersIfNoCombo, ignoreKeys } = {
    ...
    const mod = KEY_MODS[computeHashIdOfEvent(evt)];
