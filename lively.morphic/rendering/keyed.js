// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
// THIS CODE STEMS FROM STAGE0 AND WAS LICENSED UNDER MIT.
// COPIED IN 05/2022 FROM https://github.com/Freak613/stage0.
// MINOR MODIFICATIONS TO MAKE OUR LINTER HAPPY.
// lh 2022-05-18. 
// -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-
...
export function keyed (key, parent, renderedValues, data, createFn, noOp = noOpUpdate, beforeNode, afterNode) {
  ...
  let newStart = 0;
  let loop = true;
  let prevEnd = renderedValues.length - 1; let newEnd = data.length - 1;
  let a; let b;
  ...
  let lisIdx = longestSeq.length - 1; let tmpD;
