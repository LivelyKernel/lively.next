...
export default class Renderer {
  ...
  ensureDefaultCSS () {
    ...
    return promise.waitFor(3000, () => this.bodyNode.getRootNode())
      .then(doc => Promise.all([
        ...
        addOrChangeLinkedCSS('lively-material-icons', config.css.materialIcons, doc, false),
  ...
  renderStructuralChanges (morph) {
    ...
    if (submorphsToRender.length === 0) {
      ...
      morph.renderingState.hasMorphRemoved = false;
      return;
    }
  ...
  renderSelectionPart (morph, selection, diminished = false, cursorWidth = 2) {
    ...
    while (row <= selection.end.row) {
      ...
      if (isWrapped) {
        ...
        const rangesToRender = textLayout.rangesOfWrappedLine(morph, row).map(r => r.intersect(selection)).filter(r => !r.isEmpty());
        ...
        for (const r of rangesToRender) {
          ...
          subCharBounds = charBounds.slice(r.start.column, r.end.column + (isMultiline ? 1 : 0));
