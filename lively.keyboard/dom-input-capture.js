...
export default class DOMInputCapture {
  ...
  install (newRootNode) {
    ...
    let doc = typeof window !== 'undefined' && newRootNode === window
      ? window.document
      : newRootNode.ownerDocument;
    let { isInstalled, rootNode } = domState;
  ...
  resetValue () {
    let n = this.domState.textareaNode;
  ...
  async execCommand (stateName, execFn) {
    ...
    let deferred = promise.deferred(); let isDone = false;
    state[stateName] = {
      onEvent: evt => {
        if (isDone) return;
  ...
  setPosition (pos) {
    let { textareaNode } = this.domState || {};
  ...
  ensureBeingAtCursorOfText (textMorph) {
    ...
    let posInClippedBounds = textMorph.innerBounds().constrainPt(localCursorPos);
    let globalCursorPos = textMorph.worldPoint(posInClippedBounds.subPt(textMorph.scroll));
  ...
  onRootNodeFocus (evt) {
    let { textareaNode, rootNode } = this.domState || {};
  ...
  onCompositionUpdate (evt) {
    let { composition: c } = this.inputState;
