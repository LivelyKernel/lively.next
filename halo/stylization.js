...
class StyleHalo extends Morph {
  ...
  get targetBounds() {
    return this.target.globalPosition
  ...
  onMouseDown(evt) {
    if (!this.get("borderHalo").onMouseDown(evt)
  ...
  intersectionShape() {
    const rx = this.target.borderRadius, {width, height} = this.target;
  ...
  isOnMorphBorder(evt) {
    ...
    const {x, y} = evt.positionIn(this.submorphs[0]),
          ...
          otherShape = shape("rect", {x: x - bw, y: y - bw, width: bw * 2, height: bw * 2});

    return !brHalo.active && intersect(this.intersectionShape(), otherShape).points.length > 0;
  }
   ...
   openBorderStyler() {
    ...
    this.world().addMorph(
      this.borderStyler.createLeashFor(this.borderHalo, this.getSideInWorld()),
      this);
  ...
  borderHaloShape(props) {
    ...
    return {
      ...
      activate() {
        this.active = true;
        this.borderColor = Color.orange.withA(0.4);
      },
      selectBorder() {
        if (!this.active)
          return;
  ...
  getBorderHalo() {
    ...
    this.borderHalo = morph(
      this.borderHaloShape({
        name: "borderHalo",
        update(evt) {
  ...
  borderRadiusHalo() {
    ...
    return {
      ...
      onHoverOut(evt) {
        if (evt.state.draggedMorph != this)
          this.active = false;
      },
      update(evt) {
        this.center = getPos();
      ...
      onDragStart(evt) {
        ...
        this.borderRadiusView = this.addMorph(new Text({
          ...
          position: pt(10, 10),
          textString: halo.target.borderRadius + "px"
        }));
      },
      onDrag(evt) {
...
export class EllipseStyleHalo extends StyleHalo {
  ...
  borderHaloShape(props) {
    const halo = this;
    return {...super.borderHaloShape(props), type: "ellipse"};
  }
    ...
    return [
      new Leash({
        ...
        update() {
          ...
          if (this.vertices[1].position.equals(pos))
            return;
...
class SvgStyleHalo extends StyleHalo {
  ...
  getBorderStyler() {
    const borderStyler = new PolygonEditor({
      target: this.target,
      title: "Change Border Style"
    });
    ...
    connect(
      borderStyler, "transform vertices",
      this, "startTransformingVertices");
    return borderStyler;
  }
  ...
  initVertexHandles() {
    ...
    this.vertexHandles = this.target.vertices.map(({x, y}, i) => {
      return bh.addMorph(new VertexHandle({
        halo,
        position: pt(x, y),
        index: i
      }));
    ...
    return {
      ...
      submorphs: [
        {
          ...
          onMouseDown() {
            ...
            arr.pushAt(
              halo.target.vertices,
              this.position.addXY(-bw, -bw),
              this.insertionIndex
            );
      ...
      showHandlePlaceholder(pos) {
        ...
        var vs = halo.target.vertices,
          [v0, v1] = arr.min(
            arr.zip(vs, arr.rotate(vs)),
            ([a, b]) => pos.dist(pointOnLine(a, b, pos, bw))
          ),
      ...
      alignWithTarget() {
        ...
        if (!halo.target.borderWidth)
          this.moveBy(pt(-1, -1));
        if (halo.borderStyler.opened)
