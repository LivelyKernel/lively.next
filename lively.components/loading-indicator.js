...
export default class LoadingIndicator extends Morph {
  static open (label, props) {
    const li = morph({ type: LoadingIndicator, ...props, label });
  ...
  async hideSpinner () {
    if (!this.ui.spinner) return;
    this.ui.spinner.visible = false;
    this.ui.spinner.width = 50;
  ...
  updateLabel () {
    const center = this.center; this.relayout();
    this.center = center;
  }

  relayout () {
    const padding = Rectangle.inset(20, 12);
    const { spinner, label, closeButton, progressBar, status } = this.ui;
