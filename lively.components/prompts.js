...
import {
  morph, VerticalLayout, Morph, StyleSheet, Text, GridLayout,
  Icon, HorizontalLayout, InputLine, PasswordInputLine
} from 'lively.morphic';
...
export class AbstractPrompt extends Morph {
  ...
  async activate () {
    ...
    if (this.autoRemove) { promise.finally(this.answer.promise, () => this.fadeOut(200)); }
  ...
  addNamed (name, spec) {
    const m = this.getSubmorphNamed(name) || this.addMorph(spec.isMorph ? spec : morph({ ...spec, name }));
...
export class InformPrompt extends AbstractPrompt {
  ...
  build (props = {}) {
    const { label, lineWrapping } = props;
    const title = this.addNamed('promptTitle', { type: 'text', lineWrapping });
    const okButton = this.addNamed('ok button', {
...
export class ConfirmPrompt extends AbstractPrompt {
  ...
  build (props) {
    ...
    const cancelButton = this.addNamed('cancel button', {
      type: 'button',
...
export class MultipleChoicePrompt extends AbstractPrompt {
  ...
  static get properties () {
    return {
      ...
      choices: {
        ...
        set (choices) {
          let choiceContainer;
          if (choiceContainer = this.getSubmorphNamed('choices')) { choiceContainer.choices = choices; }
        }
      },
      master: {
        initialize () {
  ...
  onKeyDown (evt) {
    if (/^[0-9]$/.test(evt.keyCombo)) {
      const n = Number(evt.keyCombo) - 1;
      const btn = this.getSubmorphNamed('button ' + n);
      if (btn) {
        btn.trigger();
        return evt.stop();
      }
...
export class TextPrompt extends AbstractPrompt {
  ...
  resolve () {
    const inputLine = this.getSubmorphNamed('input');
  ...
  build ({ label, input, historyId, useLastInput }) {
    ...
    const inputLine = this.addNamed('input', Text.makeInputLine({
      historyId,
  ...
  initLayout () {
    ...
    const input = this.getSubmorphNamed('input');
    label.fit();
...
export class EditPrompt extends AbstractPrompt {
  ...
  build ({ label, input, historyId, useLastInput, mode, textStyle, evalEnvironment, resolveTextAttributes = false }) {
    ...
    inputEditor.changeEditorMode(mode).then(() => {
      if (evalEnvironment && inputEditor.editorPlugin) { Object.assign(inputEditor.editorPlugin.evalEnvironment, evalEnvironment); }
    });

    const inputWidth = inputEditor.textBounds().width + 50;
    // if the input string we pre-fill is wide than we try to make it fit
    if (inputWidth > this.width - 25) { this.width = Math.min(this.maxWidth, inputWidth + 125); }
...
export class ListPrompt extends AbstractPrompt {
  ...
  static get properties () {
    return {
      items: {
        ...
        after: ['submorphs'],
        set (items) {
          if (this.submorphs.length > 0) { this.getSubmorphNamed('prompt list').items = items || []; }
        }
      },
      filterable: {},
      master: {
  ...
  build ({
    ...
    const list = this.addNamed('prompt list', {
    ...
    if (onSelection && typeof onSelection === 'function') { this.onSelection = onSelection; }
...
export class EditListPrompt extends ListPrompt {
  ...
  build (props) {
    ...
    const addBtn = this.addNamed('add item button', {
      type: 'button',
      label: Icon.makeLabel('plus')
    });
    const rmBtn = this.addNamed('remove item button', {
      type: 'button',
  ...
  resolve () {
    const { values: list, selections } = this.get('prompt list');
