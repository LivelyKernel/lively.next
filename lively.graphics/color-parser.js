...
const absround = function (number) {
  return (0.5 + number) << 0;
};

const hue2rgb = function (a, b, c) {
  if (c < 0) c += 1;
  if (c > 1) c -= 1;
...
Color.prototype.getRGB = function () {
  const components = [absround(this._red), absround(this._green), absround(this._blue)];
  return 'rgb(' + components.join(', ') + ')';
};

Color.prototype.getPRGB = function () {
  const components = [absround(100 * this._red / 255) + '%', absround(100 * this._green / 255) + '%', absround(100 * this._blue / 255) + '%'];
  return 'rgb(' + components.join(', ') + ')';
};

Color.prototype.getRGBA = function () {
  const components = [absround(this._red), absround(this._green), absround(this._blue), this._alpha];
  return 'rgba(' + components.join(', ') + ')';
};

Color.prototype.getPRGBA = function () {
  const components = [absround(100 * this._red / 255) + '%', absround(100 * this._green / 255) + '%', absround(100 * this._blue / 255) + '%', this._alpha];
  return 'rgba(' + components.join(', ') + ')';
};
