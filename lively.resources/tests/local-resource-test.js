...
describe('local resource', function () {
  ...
  it('can read a file', async () => {
    ...
    let r = resource(`local://${host}/test-location/foo`);
    await r.read();
  ...
  describe('file listing', async () => {
    ...
    it('of subdirectory recursively', async () => {
      let r = resource(`local://${host}/sub-dir`);
  ...
  xdescribe('file props', () => {
    it('retrieves file props', async () => {
      let { size, lastModified, contentType } = await r1.readProperties();
    ...
    it('with dirList', async () => {
      let r = resource(`local://${host}/test-location/`);
      let file1 = (await r.dirList())[0];
      let { size, lastModified } = file1;
