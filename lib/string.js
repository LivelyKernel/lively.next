...
var string = exports.string = {
  ...
  minIndent: function(str, indentString) {
    ...
    // Example:
    //   string.minIndent("    Hello", "  ") // => 2
    if (!indentString) indentString = "  ";
    var indentRe = new RegExp("^(" + indentString + ")*", "gm");
    return exports.arr.min(
      str.match(indentRe)
        .map(function(ea) { return Math.floor(ea.length / indentString.length); }))
  },
  ...
  changeIndent: function(str, indentString, depth) {
    ...
    if (existingIndent < depth) return string.indent(str, indentString, depth - existingIndent);
    var prefixToRemove = indentString.repeat(existingIndent - depth)
    return string.lines(str)
  ...
  lineNumberToIndexesComputer: function(s) {
    ...
    // idxComp(1) // => [6,12]
    return function(lineNo) { return string.lineRanges(s)[lineNo]; }
  },
  ...
  startsWith: features.startsWith ?
