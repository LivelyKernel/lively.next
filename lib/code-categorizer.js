...
function classDef(node) {
  if (Path("expression.callee.property.name").get(node) !== "subclass") return null;
  var def = {
...
function extendDef(node) {
  if (Path("expression.callee.property.name").get(node) !== "extend"
   || Path("expression.arguments.0.type").get(node) !== "ObjectExpression") return null;
  ...
  var props = (objectKeyValsAsDefs(Path("expression.arguments.1").get(node)) || [])
...
function moduleDef(node, options) {
  ...
  var decls = findDecls(Path("expression.arguments.0").get(node), options),
      parent = {node: node, name: Path("expression.callee.object.callee.object.arguments.0.value").get(node)};
...
function isModuleDeclaration(node) {
  return Path("expression.callee.object.callee.object.callee.name").get(node) === "module"
      && Path("expression.callee.property.name").get(node) === "toRun";
}
