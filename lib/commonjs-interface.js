...
var __moduleCache = Module._cache,
    __require = (name, parent) => Module._load(name, parent),
    __resolveFilename = (name, parent) => Module._resolveFilename(name, parent);

import * as evaluator from "./evaluator";
import { isAbsolute, relative, dirname, join } from "path";
import * as lang from "lively.lang";
import callsite from "callsite";
import * as fs from "fs";

// FIXME!!!
var __dirname = join(process.cwd(), "lib");
var __filename = join(__dirname, "commonjs-interface.js");
...
function resolve(file, parent) {
  ...
  // normal resolve
  try {
    return __resolveFilename(file, parent);
  } catch (e) {}
  ...
  if (!isAbsolute(file) && parent) {
    ...
    try {
      return __resolveFilename(join(parent.id, "..", file));
    } catch (e) {}
  }
  ...
  for (var i = 2; i < frames.length; i++) {
    ...
    var dir = dirname(frameFile),
        full = join(dir, file);
    try { return __resolveFilename(full, parent || module); } catch (e) {}
  }

  // last resort: current working directory
  try {
    return __resolveFilename(join(process.cwd(), file), parent || module);
  } catch (e) {}
...
function isLoaded(fileName) { return __moduleCache[fileName] && fileName in loadedModules; }
...
function customCompile(content, filename) {
  ...
  if (!__moduleCache[filename] && loadedModules[filename])
...
function runEval(code, options) {
  ...
  return Promise.resolve().then(() => {
    ...
    var m = __moduleCache[fullName],
        env = envFor(fullName),
        rec = env.recorder,
        recName = env.recorderName;
    rec.__filename = m.filename;
    var dirname = rec.__dirname = dirname(m.filename);
...
function importCjsModule(name) {
  return new Promise(ok => ok(__require(resolve(name))));
}
