...
function constructorTemplate(name) {
  ...
  return funcExpr({id: name ? id(name) : null}, [],
    varDecl("firstArg", member("arguments", literal(0))),
    ifStmt(
      binaryExpr(
        id("firstArg"),
        "&&",
        member("firstArg", funcCall(member("Symbol", "for"), literal("lively-instance-restorer")), true)),
      block(),
      block(
        exprStmt(
          funcCall(
...
class ClassReplaceVisitor extends Visitor {
  ...
  accept(node, state, path) {
    if (isFunctionNode(node)) {
      state = {...state, classHolder: objectLiteral([])}
    }
  ...
  return funcCall(
            member(
              funcCall(
                ...
                replaceSuper(node.callee.object, state.classHolder, [], options),
...
function replaceDirectSuperCall(node, state, path, options) {
  ...
  return funcCall(
            member(
              funcCall(
                member(options.functionNode, "_get"),
                replaceSuper(node.callee, state.classHolder, [], options),
...
function replaceSuperGetter(node, state, path, options) {
  ...
  return funcCall(
          member(options.functionNode, "_get"),
          replaceSuper(node.object, state.classHolder, [], options),
...
function replaceSuperSetter(node, state, path, options) {
  ...
  return funcCall(
          ...
          replaceSuper(node.left.object, state.classHolder, [], options),
  ...
  var classCreator =
    funcCall(
      funcExpr({}, ["superclass"],
        ...
        varDecl(tempLivelyClassVar, classId ?
          binaryExpr(member(tempLivelyClassHolderVar, classId), "||", constructorTemplate(classId.name)) :
          constructorTemplate(null)),
        returnStmt(
          funcCall(
