...
function customInstantiate(proceed, load) {
  if (exceptions.some(exc => exc(load.name))) {
    return proceed(load);
  }

  // if (!_currentSystem.get("@system-env").node || load.metadata["lively.vm instrumented"]) return proceed(load);
  if (!_currentSystem.get("@system-env").node) return proceed(load);

  var m = getCachedNodejsModule(load);
  if (!m) return proceed(load);

  console.log("%s is already loaded as nodejs module!", load.name);

  return {
    deps: [],
    execute: function() {
      console.log(arguments)
      console.log("RUNNING EXECUTE FOR %s", load.name);
      console.trace()
      return Promise.resolve(_currentSystem.newModule(m.exports));
    },
  };
  // try {
  //   var Module = require("module").Module,
  ...
  //     // debug && console.log("[lively.vm es6 customTranslate] loading %s from nodejs module cache", load.name);
  //     // console.log(`............... CUSTOM INSTANTIATE OF ${id}`)
  //     // console.log(load)
  //     return proceed({
  //       deps: [],
  //       execute: () => {
  //         console.log("............... CUSTOM INSTANTIATE OF " + id);
  //         return Promise.resolve(_currentSystem.newModule({}));
  //       }
  //     })
  //   } else {
  //     debug && console.log("[lively.vm es6 instantiate] %s not yet in nodejs module cache", load.name);
  //   }
  // } catch (e) {
  //   debug && console.log("[lively.vm es6 instantiate] %s unknown to nodejs", load.name);
  // }
  // return proceed(load);
}
...
function wrapModuleLoad() {
  ...
  // if (!_currentSystem.origInstantiate) {
  //   _currentSystem.origInstantiate = _currentSystem.instantiate
  //   _currentSystem.instantiate = function(load) {
  //     return customInstantiate(_currentSystem.origInstantiate.bind(_currentSystem), load);
  //   }
  // }
}
...
function unwrapModuleLoad() {
  ...
  if (_currentSystem.origInstantiate) {
    _currentSystem.instantiate = _currentSystem.origInstantiate;
    delete _currentSystem.origInstantiate;
  }
}
