...
function addGetterSettersForNewVars(moduleId, env) {
  ...
  Object.keys(env).forEach(key => {
    ...
    env.__defineSetter__(key, (v) => {
      updateModuleExportsOnVarChange(moduleId, key, v);
...
function updateModuleExportsOnVarChange(fullname, name, value) {
  var rec = moduleRecordFor(fullname);
  if (rec && name in rec.exports) updateModuleExports(fullname, name, value);
}
...
function prepareCodeForCustomCompile(source, fullname, env) {
  ...
  var tfmOptions = {
      ...
      footer = `\nSystem.__lively_vm__.evaluationDone("${fullname}", ${env.recorderName});`;
      ...
      return Promise.all(updateData.localDeps.map(depName => {
        ...
        return depModule && record ?
          ...
          importES6Module(depFullname).then((module) =>
            ({name: depName, fullname: depFullname, module: module, record: moduleRecordFor(depFullname)}));
      }))
      .then(deps => {
...
function forgetModule(moduleName) {
  var id = forgetModuleDeps(moduleName);
  currentSystem().delete(id);
  forgetEnvOf(id);
}

function reloadModule(moduleName, reloadDependencies) {
  if (reloadDependencies === undefined) reloadDependencies = true
  var id = resolve(moduleName),
      deps = findDependentsOf(id),
      all = deps.concat([id]);
  deps.forEach(ea => {
  ...
  return (reloadDependencies ?
          Promise.all(deps.map(ea => importES6Module(ea))) :
          Promise.resolve())
