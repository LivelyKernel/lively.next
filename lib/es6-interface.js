      ...
      _moduleImport: {
        get: function() {
          return (moduleName, name) => {
            console.log("IMPPORT NOT YET IMPLEMENTED");
          }
          // var depMap = System.loads[fullname].depMap,
          //     localModules = Object.keys(depMap).reduce((imported, modName) => {
          //       var fullname = depMap[modName];
          //       imported[modName] = System._loader.modules[fullname].module;
          //       return imported;
          //     }, {});
          // return localModules;
        }
...
function updateModuleExports(fullname, name, value) {
  updateModuleRecordOf(fullname, (record) => {
    console.log("[lively.vm es6 updateModuleExports] %s export %s = %s", fullname, name, String(value));
  ...
  return Promise.resolve().then(() => {
    ...
    return importES6Module(fullname)
      ...
      .then((m) => {
        ...
        code = `var _moduleExport = __lvVarRecorder._moduleExport;\n${code}`;
...
function sourceChange(moduleName, newSource, options) {
  ...
  return _systemTranslateParsed(load)
    .then(updateData => {
      var record = moduleRecordFor(fullname);
      record.dependencies = updateData.deps;
      var _exports = (name, val) => updateModuleExports(fullname, name, val),
          registered = updateData.setterAndExecuteBuilder(_exports);
      // run setters to populate imports
      record.dependencies.forEach((d,i) => registered.setters[i](d.exports));
      // execute module body
      registered.execute()
    })
...
function _systemTranslateParsed(load) {
  ...
  return System.translate(load).then(translated => {
    ...
    var setterAndExecuteBuilderNode = call.callee.body.body[0].expression["arguments"][1]
    var setterAndExecuteBuilderSource = translated.slice(setterAndExecuteBuilderNode.start, setterAndExecuteBuilderNode.end)
    var setterAndExecuteBuilder = eval(`var __moduleName = "${moduleName}";(${setterAndExecuteBuilderSource});`)
    var deps = depNames.map(depName => moduleRecordFor(depName));
    return {
      deps: deps,
      setterAndExecuteBuilder: setterAndExecuteBuilder
    }
  });
}
