...
function runEval(code, options) {
  ...
  options = lang.obj.merge({currentModule: null, fromModule: null, printed: null}, options);

  return Promise.resolve().then(() => {
    // if (!options.currentModule) return reject(new Error("options.currentModule not defined"));
    if (!options.currentModule) {
      options.currentModule = scratchModule;
    } else if (options.fromModule) {
      options.currentModule = path.join(options.fromModule, "..", options.currentModule);
    }
