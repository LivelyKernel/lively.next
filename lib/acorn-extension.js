...
import * as acorn_module from "acorn";
import * as walk_module from "acorn/../walk.js";
import * as loose_module from "acorn/../acorn_loose.js";
import { withMozillaAstDo } from "./mozilla-ast-visitor-interface.js";

var walk = lively.lang.obj.extend({}, walk_module);
var loose = lively.lang.obj.extend({}, loose_module);
var acorn = lively.lang.obj.merge({walk: walk, loose: loose}, acorn_module);

export { acorn, walk, loose }
...
walk.findSiblings = function(parsed, node, beforeOrAfter) {
  if (!node) return [];
  var nodes = walk.findNodesIncluding(parsed, node.start),
...
;(function extendAcornWalk2() {
  ...
  walk.findNodeByAstIndex = function(parsed, astIndexToFind, addIndex) {
    ...
    if (!parsed.astIndex && addIndex) walk.addAstIndex(parsed);
    // we need to visit every node, walk.forEachNode is highly
  ...
  // Global.findNodeByAstIndex = walk.findNodeByAstIndex;
  ...
  walk.findStatementOfNode = function(options, parsed, target) {
    ...
    if (!options) options = {}
    if (!parsed.astIndex) walk.addAstIndex(parsed);
