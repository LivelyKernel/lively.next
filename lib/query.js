...
function declarationsOfScope(scope, includeOuter, result = []) {
  ...
  result.push(...scope.funcDecls.map(ea => ea.id));
  helpers.varDeclIds(scope, result);
  result.push(...scope.catches);
  result.push(...scope.classDecls.map(ea => ea.id));
...
function __varDeclIdsFor(scope, name) {
  ...
  for (var ea of scope.funcDecls) if (ea.id.name === name) result.push(ea.id);
  for (var ea of scope.classDecls) if (ea.id.name === name) result.push(ea.id);
