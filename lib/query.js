...
function resolveReference(ref, scopePath) {
  ...
  const decl = decls.find(d => d.name == ref);
...
function resolveReferences(scope, outerScopes = []) {
  ...
  scope.refs.forEach(ref => {
    ref.decl = obj.deepCopy(resolveReference(ref.name, path));
  });
...
function refAt(pos, scope) {
  const ref = scope.refs.find(r => r.start <= pos && pos <= r.end);
  return ref || scope.subScopes.reduce((p,s) => p || refAt(pos, s), null);
}
  ...
  const exports = scope.exportDecls.reduce((exports, node) => {
    ...
    if (exportsStmt.declaration) {
      return exports.concat({
        ...
        fromModule:      null,
        node:            node,
        declaration:     exportsStmt.declaration
      })
    }
    return exports;
  }, []);
