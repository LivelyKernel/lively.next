...
var knownGlobals = [
  ...
  "Node", "HTMLCanvasElement", "Image", "lively"]
...
function imports(scope) {
  ...
  // like import "fooo";
  const importStatementsWithoutSpecifiers =
    (scope.node.body || []).filter(node =>
      node.type === "ImportDeclaration" && node.specifiers.length === 0)
        .map(node => ({local: null, imported: null, fromModule: node.source.value, node}))

  // like import { x as y } from "fooo"; import * as x from "fooo"; import x from "fooo";
  const imports = scope.importSpecifiers.reduce((imports, node) => {
