...
function blockInliner(node) {
  ...
  if (Array.isArray(node.body)) {
    node.body = node.body.reduce((body, node) => {
...
function additionalIgnoredDecls(parsed, options) {
  ...
  return []
    ...
    .concat(ignoreDecls.reduce((all, ea) => { all.push.apply(all, ea.declarations); return all; }, []).map(ea => ea.id.name));
}
...
function shouldRefBeCaptured(ref, toplevel, options) {
  return !arr.include(toplevel.scope.importDecls, ref)
    && !arr.include(arr.flatmap(toplevel.scope.exportDecls, ea => ea.declarations ? ea.declarations : ea.declaration ? [ea.declaration] : []), ref)
...
function insertCapturesForExportDeclarations(parsed, options) {
  parsed.body = parsed.body.reduce((stmts, stmt) => {
    ...
    if ((stmt.type !== "ExportNamedDeclaration" && stmt.type !== "ExportDefaultDeclaration") || !stmt.declaration) return stmts;
    if (stmt.declaration.declarations) {
      stmts.push(...stmt.declaration.declarations.map(decl =>
...
function insertDeclarationsForExports(parsed, options) {
  ...
  parsed.body = parsed.body.reduce((stmts, stmt) => {
    if (stmt.type === "ExportDefaultDeclaration" && stmt.declaration && stmt.declaration.type.indexOf("Declaration") === -1) {
      ...
      return stmts.concat(stmt.specifiers.map(specifier =>
