  ...
  */
  ...
  var parsed = typeof astOrSource === 'object' ?
      ...
      source = typeof astOrSource === 'string' ?
        astOrSource : (parsed.source || escodegen.generate(parsed)),
      rewritten = parsed;
...
// console.log(escodegen.generate(rewritten));
  // console.log(require("util").inspect(rewritten.body, {depth: 10}));

  return {
    ast: rewritten,
    source: escodegen.generate(rewritten),
    defRanges: defRanges
  }
}
...
function replace(parsed, replacer) {
  ...
  v.accept = fun.wrap(v.accept, (proceed, node, state, path) => replacer(proceed(node, state, path)));
