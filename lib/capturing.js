...
function shouldRefBeCaptured(ref, toplevel, options) {
  return !toplevel.scope.importDecls.includes(ref)
      && !arr.flatmap(toplevel.scope.exportDecls, ea => ea.declarations ? ea.declarations : ea.declaration ? [ea.declaration] : []).includes(ref)
      && !options.excludeRefs.includes(ref.name)
      && (!options.includeRefs || options.includeRefs.includes(ref.name));
}
