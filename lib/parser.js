...
import { acorn, loose, walk } from "./acorn-extension.js";
...
function parse(source, options) {
  ...
  var ast = options.addSource ?
    walk.addSource(source, options) : // FIXME
    acorn.parse(source, options);

  if (options.addAstIndex && !ast.hasOwnProperty('astIndex')) walk.addAstIndex(ast);
...
function parseFunction(source, options) {
  ...
  var src = '(' + source + ')',
    ast = acorn.parse(src);
  /*if (options.addSource) */walk.addSource(ast, src);
...
function fuzzyParse(source, options) {
  ...
  try {
    ...
    else if (options.addSource) walk.addSource(ast, source);
  } catch (e) { err = e; }
  ...
  if (!ast) {
    ...
    if (options.addSource) walk.addSource(ast, source);
    ast.isFuzzy = true;
    ast.parseError = err;
  }
  return ast;
